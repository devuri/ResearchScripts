...
"""
This program gets separate opinion text segments from the full judgments. It reads the full judgments lines in reverse
because separate opinions always come at the end.
"""
...
for file in glob.glob("*.txt"):
    ...
    if not itemid in ['001-107022','001-156274','001-161051','001-57524','001-57886','001-75450','001-75511','001-89583','001-92999','001-98550','001-162211','001-61696']:
        ...
        #reverse the list because separate opinions are always at the end
        lines.reverse()
        #foundConclusion is a flag for whether we have reached the conclusion yet (filter out previous info)
        foundConclusion = False
        #foundDissent is a flag for whether we found "separate" in the text segment
        foundDissent = False
        ...
        for line in lines:
            lineNum += 1
            if foundConclusion: #break out of loop if we've found the conclusion and have read all separate opinions
                break
            ...
            if "SEPARATE OPINION" in originalLine or "Separate OPINION" in originalLine:
                foundDissent = True
        ...
        if foundDissent:
            ...
            for line in textSegmentsAsc:
                ...
                if processingAppendix and "separate opinion" in line:
                    processingAppendix = False
                    ...
                    if "OPINION" in textSegments[index] or "STATEMENT" in textSegments[index] or "OPINON" in textSegments[index] or "OPIONION" in textSegments[index]:
                        ...
                        for case in caseList:
                            if case['itemid'] == itemid:
                                if "separate opinion" in textSegmentsAsc[index]:
                                    dissentNum += 1
...
sheet = workbook.add_sheet("SeparateOpinions")
sheet.write(0,0,"itemid")
sheet.write(0,1,"separate opinion number")
sheet.write(0,2,"Judge Names")
sheet.write(0,3,"Judge Last Names")
sheet.write(0,4,"Number of Judges")
sheet.write(0,5,"Separate Opinion Text")
...
for dissentingOpinion in dissentList:
    ...
    if len(dissentingOpinion['dissentText']) <= 32767:
        ...
        sheet.write(row,5,"OVERFLOW - separate opinion has too many characters to fit in one cell, please see reference")
        """
