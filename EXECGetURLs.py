"""
This program parses a local html file containing the "hidden" HTML from the HUDOC Executions search results page and extracts
...
BASE_URL = "http://hudoc.exec.coe.int/eng?i=" #+item id

#HOME_FILE is the location of the local html file
HOME_FILE = "SearchResults.html" #Fill in with location of html file that contains urls
...
def get_report_links(file_name):
    ...
    for item in soup.find_all("div", {"style": "display:none;"}):
        count += 1
        print BASE_URL + ((item.get_text().splitlines()[3])[16:])
