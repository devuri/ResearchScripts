...
"""
This program gets concurring opinion text segments from the full judgments. It reads the full judgments lines in reverse
because concurring opinions always come at the end.
"""
...
JUDGE_NAME_FILE = '' #Fill in with location of text file of judge names
...
#Location of directory containing English full judgments (>11000 files)
FULL_JUDGMENTS_FILES = '' #Fill in with location of folder of court judgments written in English
...
for file in glob.glob("*.txt"):
    ...
    if not itemid in ['001-107022','001-156274','001-161051','001-57524','001-57886','001-75450','001-75511','001-89583','001-92999','001-98550','001-162211','001-61696']:
        ...
        #reverse the list because concurring opinions are always at the end
        lines.reverse()
        #foundConclusion is a flag for whether we have reached the conclusion yet (filter out previous info)
        foundConclusion = False
        #foundDissent is a flag for whether we found "concurring" in the text segment
        foundDissent = False
        ...
        for line in lines:
            lineNum += 1
            if foundConclusion: #break out of loop if we've found the conclusion and have read all concurring opinions
                break
            ...
            if "CONCURRING OPINION" in originalLine:
                foundDissent = True
        ...
        if foundDissent:
            ...
            for line in textSegmentsAsc:
                ...
                if processingAppendix and "concurring opinion" in line:
                    processingAppendix = False
                if not processingAppendix:
                    ...
                    if "OPINION" in textSegments[index] or "STATEMENT" in textSegments[index] or "OPINON" in textSegments[index] or "OPIONION" in textSegments[index]:
                        ...
                        for case in caseList:
                            if case['itemid'] == itemid:
                                if "concurring opinion" in textSegmentsAsc[index]:
                                    dissentNum += 1
                    ...
                    o.write('\n')
                index += 1
            #append last concurring opinion of full judgment
            if dissentProfile['dissentNum'] != -1:
                dissentList.append(dissentProfile)
            o.write('--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n')
o.close()
...
sheet = workbook.add_sheet("ConcurringOpinions")
sheet.write(0,0,"itemid")
sheet.write(0,1,"concurring opinion number")
sheet.write(0,2,"Judge Names")
sheet.write(0,3,"Judge Last Names")
sheet.write(0,4,"Number of Judges")
sheet.write(0,5,"Concurring Opinion Text")
...
for dissentingOpinion in dissentList:
    ...
    if len(dissentingOpinion['dissentText']) <= 32767:
        ...
        sheet.write(row,5,"OVERFLOW - concurring opinion has too many characters to fit in one cell, please see reference")
        """
        ...
        """
    row += 1
workbook.save("") #Fill in with location of excel spreadsheet to save